...
class ResNetFc(nn.Module):
  ...
  def forward(self, x, cam=False):
    ...
    #if cam:
    #    cam = F.conv2d(x, self.fc.weight.unsqueeze(-1).unsqueeze(-1)).detach()
...
class AddaNet(nn.Module):
    ...
    def setup_net(self):
        ...
        self.src_net = models[self.base_model]()
        self.tgt_net = models[self.base_model]()
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        #x = nn.Linear(x.size(1),256).cuda()(x)
...
class Generator(nn.Module):
    def __init__(self, input_size, num_feature):
        ...
        self.fc = nn.Linear(input_size, 1024)  # batch,1024
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0),-1)
        #x = nn.Linear(x.size(1), 512).cuda()(x)
        return x
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.upsample1 = nn.Sequential(
            nn.ConvTranspose2d(1, 25, 2, stride=2),  # 从 (1, 16, 16) 反向卷积到 (25, 32, 32)
            nn.BatchNorm2d(25),
            nn.ReLU(True)
        )
        self.upsample2 = nn.Sequential(
            nn.ConvTranspose2d(25, 50, 3, stride=1, padding=1),  # 从 (25, 32, 32) 反向卷积到 (50, 32, 32)
            nn.BatchNorm2d(50),
            nn.ReLU(True)
        )
        self.upsample3 = nn.Sequential(
            nn.ConvTranspose2d(50, 1, 3, stride=1, padding=1),  # 从 (50, 32, 32) 反向卷积到 (1, 32, 32)
            nn.Tanh()
        )

    def forward(self, x):
        x = x.view(x.size(0), 1, 16, 16)  # 将特征变形为 (1, 16, 16) 的形状
        x = self.upsample1(x)
        x = self.upsample2(x)
        x = self.upsample3(x)
        return x
...
class Discriminator_um(nn.Module):
    def __init__(self):
        super(Discriminator_um, self).__init__()
...
class Generator_um(nn.Module):
    def __init__(self, input_size, num_feature):
        super(Generator_um, self).__init__()
        self.fc = nn.Linear(input_size, 1024)  # batch,1024
        self.br = nn.Sequential(
            nn.BatchNorm2d(1),
            nn.ReLU(True)
...
class Discriminator_um_pixel(nn.Module):
    def __init__(self):
        super(Discriminator_um_pixel, self).__init__()
...
class Generator_um_pixel(nn.Module):
    def __init__(self, input_size, num_feature):
        super(Generator_um_pixel, self).__init__()
        self.fc = nn.Linear(input_size, 3136)  # batch,3136
        self.br = nn.Sequential(
            nn.BatchNorm2d(1),
            nn.ReLU(True)
        )
        ...
        self.downsample3 = nn.Sequential(
            nn.Conv2d(25, 2, 2, stride=2),  # batch, 2, 28, 28
            nn.Tanh()
        )
...
models={
    ...
    "Generator":Generator,
    "Decoder":Decoder,
    "Discriminator_digits": Discriminator_digits,
    "Generator_digits": Generator_digits,
    "Discriminator_um": Discriminator_um,
    "Generator_um": Generator_um,
    "Discriminator_um_pixel": Discriminator_um_pixel,
    "Generator_um_pixel": Generator_um_pixel,
    ...
    "DTN":DTN,
    "LeNet":LeNet,
    "AdversarialNetwork":AdversarialNetwork,
    "AddaNet":AddaNet
}
